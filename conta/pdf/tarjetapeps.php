...
<?php
	...
	function Header()
	{
	    ...
	    $this->SetFont('Arial','B',20);
      	...
      	$this->Cell(280,6,utf8_decode('MÉTODO PRIMERAS ENTRADAS, PRIMERAS SALIDAS (PEPS)'),0,0,'C');
      	$this->Ln(7);
	...
	function Footer()
	{
		...
		$sql = "SELECT * FROM tarjetapeps  Where nom_us= '$nombre'";
	...
	$pdf->SetFont('Times','',13);
		while ($row=$resultado->fetch_assoc()) {
				...
				$pdf->Cell(10, 10, $row['id_peps'], 1, 0, 'C',true);
				$pdf->SetFillColor(171, 235, 198  );
				$pdf->Cell(25, 10, $row['fecha_peps'], 1, 0, 'C',true);
				$pdf->SetFillColor(169, 223, 191 );
				$pdf->Cell(49, 10, $row['concepto_peps'], 1, 0, 'C',1);
				$pdf->SetFillColor(171, 235, 198  );
				$pdf->Cell(17, 10, $row['unidades_ent_peps'], 1, 0, 'C',1);
				$pdf->SetFillColor(169, 223, 191 );
				$pdf->Cell(20, 10, $row['costounitario_ent_peps'], 1, 0, 'L',1);
				$pdf->SetFillColor(171, 235, 198  );
				$pdf->Cell(20, 10, $row['total_ent_peps'], 1, 0, 'C',1);
				$pdf->SetFillColor(169, 223, 191 );
				$pdf->Cell(20, 10, '$'. $row['unidades_sal_peps'], 1, 0, 'R',1);
				$pdf->SetFillColor(171, 235, 198  );
				$pdf->Cell(30, 10, '$'.$row['costounitario_sal_peps'], 1, 0, 'R',1);
				$pdf->SetFillColor(171, 235, 198  );
				$pdf->Cell(30, 10, '$'.$row['total_sal_peps'], 1, 0, 'R',1);
				$pdf->SetFillColor(171, 235, 198  );
				$pdf->Cell(30, 10, '$'.$row['unidades_peps'], 1, 0, 'R',1);
				$pdf->Cell(30, 10, '$'.$row['total_peps'], 1, 0, 'R',1);
				$pdf->Ln();
			}	
$pdf->Ln();$pdf->Ln();
